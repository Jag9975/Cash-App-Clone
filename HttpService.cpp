...
User *HttpService::getAuthenticatedUser(HTTPRequest *request)  {
  ...
  // continue if auth_token is provided
  if (request->hasAuthToken()){
    string token = request->getAuthToken();

    // check if a user with this token exists and return it
    if(this->m_db->auth_tokens.find(token) != this->m_db->auth_tokens.end()){
      return this->m_db->auth_tokens[token];
    } else {
